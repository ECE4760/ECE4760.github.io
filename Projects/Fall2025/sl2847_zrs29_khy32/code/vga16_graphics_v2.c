...
void drawPiece(int piece, int isBlack, int x, int y, int size)
{
  if (piece < 0 || piece > 5) return;
  if (size != 1 && size != 2) return;

  /* choose sprite */
  const uint8_t (*img)[28];

  if (!isBlack) {
    switch (piece) {
      case PAWN:   img = white_pawn;   break;
      case ROOK:   img = white_rook;   break;
      case KNIGHT: img = white_knight; break;
      case BISHOP: img = white_bishop; break;
      case QUEEN:  img = white_queen;  break;
      case KING:   img = white_king;   break;
    }
  } else {
    switch (piece) {
      case PAWN:   img = black_pawn;   break;
      case ROOK:   img = black_rook;   break;
      case KNIGHT: img = black_knight; break;
      case BISHOP: img = black_bishop; break;
      case QUEEN:  img = black_queen;  break;
      case KING:   img = black_king;   break;
    }
  }
  ...
  for (int sy = 0; sy < dim; sy++) {
    ...
    for (int sx = 0; sx < dim; sx++) {
      int src_x = sx / scale;
      uint8_t px = img[src_y][src_x];
      if (px != WT && px != BT)
        drawPixel(x + sx, y + sy, px);
    }
  }
}
