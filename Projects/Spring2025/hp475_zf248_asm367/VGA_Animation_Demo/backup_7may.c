...
static inline int gfix_to_pixel(fix15 gfix)
{
    gfix = clamp(gfix, float2fix15(-1.f), float2fix15(1.f));
...
static inline void draw_paddles_core0(void)
{
    ...
    if (abs(xBase-last_xA)|abs(yBase-last_yA)|abs(tilt-last_tiltA)){
        ...
        last_xA=xBase; last_yA=yBase; last_tiltA=tilt;
        last_xTopA=xTop; last_yTopA=yTop; last_xBotA=xBot; last_yBotA=yBot;
    }
    /* --- PLAYER B --- */
    xBase=paddleState[activeBuffer].xB; yBase=paddleState[activeBuffer].yB; tilt=paddleState[activeBuffer].tiltB;
    xTop = xBase + tilt;  yTop=yBase;
    xBot = xBase - tilt;  yBot=yBase+PADDLE_H;
    if (abs(xBase-last_xB)|abs(yBase-last_yB)|abs(tilt-last_tiltB)){
        ...
        last_xB=xBase; last_yB=yBase; last_tiltB=tilt;
        last_xTopB=xTop; last_yTopB=yTop; last_xBotB=xBot; last_yBotB=yBot;
    }
}
// ------------------------------------------------------------------
//                         ball reflection
...
static PT_THREAD(protothread_imus(struct pt *pt))
{
    ...
    while (1) {
        ...
        // ------------------ PLAYER A (left) ------------------
// int xA = mapAxisToRange(filtAZ, XA_RANGE);          // Horizontal
// int yA = mapAxisToRange(-filtAX, Y_RANGE);          // Vertical (inverted)
// int tiltA = clamp(-fix2int15(multfix15(filtAZ, int2fix15(MAX_TILT_PX))), -MAX_TILT_PX, MAX_TILT_PX);
...
// int xB = XB_MIN + mapAxisToRange(filtBZ, XB_RANGE); // Horizontal
// int yB = mapAxisToRange(filtBX, Y_RANGE);           // Vertical
// int tiltB = clamp(fix2int15(multfix15(filtBZ, int2fix15(MAX_TILT_PX))), -MAX_TILT_PX, MAX_TILT_PX);
...
int main()
{
  ...
  setCursor(40, 430);
  writeString("8. The game is played to 3 sets, whoever wins 3 sets wins the game");
  setTextColor(RED);
  setTextSize(1);
  setCursor(500, 460);
