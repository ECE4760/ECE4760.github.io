...
void mpu6050_reset(uint8_t addr) {
    ...
    i2c_write_blocking(I2C_CHAN, addr, buf, 2, false);

    // Set gyro sample rate (set to 1KHz, same as accel)
    uint8_t gyro_rate[] = {0x19, 0b00000111} ;
    i2c_write_blocking(I2C_CHAN, addr, gyro_rate, 2, false);

    // Configure the Gyro range (+/- 250 deg/s)
    uint8_t gyro_settings[] = {0x1b, 0b00000000} ;
    i2c_write_blocking(I2C_CHAN, addr, gyro_settings, 2, false);

    // Configure the Accel range (+/- 2g's)
    uint8_t accel_settings[] = {0x1c, 0b00000000} ;
    i2c_write_blocking(I2C_CHAN, addr, accel_settings, 2, false);

    // Configure interrupt pin
    uint8_t pin_settings[] = {0x37, 0b00010000} ;
    i2c_write_blocking(I2C_CHAN, addr, pin_settings, 2, false);

    // Configure data ready interrupt
    uint8_t int_config[] = {0x38, 0x01} ;
    i2c_write_blocking(I2C_CHAN, addr, int_config, 2, false);
}
...
void mpu6050_read_raw(uint8_t addr, fix15 accel[3], fix15 gyro[3]) {
    ...
    uint8_t val = 0x3B;
    i2c_write_blocking(I2C_CHAN, addr, &val, 1, true); // true to keep master control of bus
    i2c_read_blocking(I2C_CHAN, addr, buffer, 6, false);
    ...
    val = 0x43;
    i2c_write_blocking(I2C_CHAN, addr, &val, 1, true);
    i2c_read_blocking(I2C_CHAN, addr, buffer, 6, false);  // False - finished with bus

    for (int i = 0; i < 3; i++) {
        temp_gyro = (buffer[i<<1] << 8 | buffer[(i<<1) + 1]);
        gyro[i] = temp_gyro ;
        gyro[i] = multfix151(gyro[i], 500<<16) ; // deg/sec
    }
}
/////////////////////////////////////////////////////////////////