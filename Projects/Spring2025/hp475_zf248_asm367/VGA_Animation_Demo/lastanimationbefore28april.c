...
static inline int gfix_to_pixel(fix15 gfix)          // g∈[-1,1] → [0,SCREEN_H-PADDLE_H]
{
    if (gfix < float2fix15(-1.f)) gfix = float2fix15(-1.f);
    if (gfix > float2fix15( 1.f)) gfix = float2fix15( 1.f);
    // (g+1)/2 × (SCREEN_H-PADDLE_H)
    fix15 tmp = multfix15((gfix + float2fix15(1.f)), float2fix15(0.5f));
    return fix2int15(multfix15(tmp, int2fix15(SCREEN_H - PADDLE_H)));
}

void update_paddles_from_imus(void)
{
    //---------------- read raw sensors ----------------
    ...
    //---------------- LPF ----------------
    // accel Y LSB = 16384 counts per 1 g ⇒ divide by 16384 (2¹⁴) → Q15 g-units
    fix15 ayA = aA[1] << 1;        // (raw << 1) ≈ raw / 16384 because raw is 16-bit
    fix15 ayB = aB[1] << 1;

    filtA = filtA + multfix15(alpha, (ayA - filtA));   // y[n] = y[n-1] + α(x-y[n-1])
    filtB = filtB + multfix15(alpha, (ayB - filtB));

    int yA = gfix_to_pixel(filtA);
    int yB = gfix_to_pixel(filtB);

    //---------------- redraw only if moved ≥1 px ----------------
    if (abs(yA - lastY_A) >= 1) {
        fillRect( 20, lastY_A,                PADDLE_W, PADDLE_H, BLACK);
        fillRect( 20, yA,                     PADDLE_W, PADDLE_H, RED);
        lastY_A = yA;
    }
    if (abs(yB - lastY_B) >= 1) {
        fillRect(SCREEN_W-20-PADDLE_W, lastY_B, PADDLE_W, PADDLE_H, BLACK);
        fillRect(SCREEN_W-20-PADDLE_W, yB,      PADDLE_W, PADDLE_H, RED);
        lastY_B = yB;
    }
}
...
// Single reflection function; extend here for physics-based behavior
// void reflectBallFromPaddle(fix15 *x, fix15 *y, fix15 *vx,fix15 *vy, int paddleX, int paddleY){
//     int ballX=fix2int15(*x), ballY=fix2int15(*y);
//     if(ballX>=paddleX && ballX<=paddleX+PADDLE_W &&
//        ballY>=paddleY && ballY<=paddleY+PADDLE_H){
//         *vx = -(*vx);
//         // future physics: adjust vy or add spin etc.
//         // Add topspin effect: adjust vertical velocity based on impact point
//         int paddleCenterY = paddleY + PADDLE_H / 2;
//         int impactOffset = ballY - paddleCenterY; // Distance from paddle center
//         *vy += int2fix15(impactOffset / 10); // Scale the offset to control spin intensity
//     }
// }
...
void reflectBallFromPaddle(fix15 *x, fix15 *y, fix15 *vx, fix15 *vy, int paddleX, int paddleY) {
  ...
  if (ballX >= paddleX && ballX <= paddleX + PADDLE_W &&
      ...
      *vy += int2fix15(impactOffset / 10); // Scale the offset to control spin intensity
      ...
      // Scale horizontal velocity based on impact point
      float speedFactor = 1.0 + fabs(impactOffset) / (PADDLE_H / 2); // Increase speed for edge hits
      *vx = multfix15(*vx, float2fix15(speedFactor));

      // Apply friction to reduce speed slightly
      *vx = multfix15(*vx, float2fix15(0.98)); // Reduce horizontal speed by 2%
      *vy = multfix15(*vy, float2fix15(0.98)); // Reduce vertical speed by 2%

      // Limit maximum speed
      fix15 maxSpeed = int2fix15(10); // Example max speed
      if (absfix15(*vx) > maxSpeed) {
          *vx = (*vx > 0) ? maxSpeed : -maxSpeed;
      }
      if (absfix15(*vy) > maxSpeed) {
          *vy = (*vy > 0) ? maxSpeed : -maxSpeed;
      }
      // Add randomness to reflection
      int randomOffset = (rand() % 3) - 1; // Random value between -1 and 1
      *vy += int2fix15(randomOffset);

      // Gradually reduce vertical velocity (spin decay)
      *vy = multfix15(*vy, float2fix15(0.99)); // Reduce spin effect slightly
      }
}
...
void wallsAndEdges(fix15 *x,fix15 *y,fix15 *vx,fix15 *vy)
{
    ...
    *vy += multfix15(ballSpin,magnusK);
    ...
    if(hitTop(*y)){ *vy=-*vy; *y+=int2fix15(5);}
    if(hitBottom(*y)){ *vy=-*vy; *y-=int2fix15(5);}
    if(hitRight(*x)){ *vx=-*vx; *x-=int2fix15(5);}
    if(hitLeft(*x)){ *vx=-*vx; *x+=int2fix15(5);}
...
static PT_THREAD(protothread_anim(struct pt *pt))
{
  ...
  while (1)
  {
    ...
    draw_paddles_core0();
    //reflectBallFromPaddle(&boid0_x, &boid0_y, &boid0_vx, &boid0_vy,  0, lastY_A);
    //reflectBallFromPaddle(&boid0_x, &boid0_y, &boid0_vx, &boid0_vy, SCREEN_W-PADDLE_W, lastY_B);
    //NEW
    reflectBallFromPaddle(&boid0_x,&boid0_y,&boid0_vx,&boid0_vy,paddleState.xA,paddleState.yA,paddleState.vXA);
    reflectBallFromPaddle(&boid0_x,&boid0_y,&boid0_vx,&boid0_vy,paddleState.xB,paddleState.yB,paddleState.vXB);
    
    // mpu6050_read_raw(accelB, gyroB);
    //printf("Accel X/Y/Z = %d, %d, %d\n", accelB[0], accelB[1], accelB[2]);
...
{
    // ----------  [NEW]  ----------
    static fix15 filtAX=0,filtAY=0, filtBX=0,filtBY=0;
    const fix15 alpha = float2fix15(0.05f);   // 0.05 ≈ LPF
    const float gyro_coeff = 0.001f;          // deg/s → g-eq
    ...
    while(1){
        ...
        // raw accel X/Y (LSB≈16384 per g) »1 to get Q15 g-units
        fix15 axA=aA[0]<<1, ayA=aA[1]<<1;
        fix15 axB=aB[0]<<1, ayB=aB[1]<<1;

        // complementary fuse gyro roll-rate for X-tilt
        fix15 gxA = (fix15)(gyro_coeff * gA[1]);  // Y-gyro
        fix15 gxB = (fix15)(gyro_coeff * gB[1]);

        filtAX += multfix15(alpha, (axA+gxA - filtAX));
        filtAY += multfix15(alpha, (ayA      - filtAY));
        filtBX += multfix15(alpha, (axB+gxB - filtBX));
        filtBY += multfix15(alpha, (ayB      - filtBY));

        int xA = fix2int15(multfix15(filtAX+float2fix15(1.f),
                                     int2fix15(160-PADDLE_W)));      // 0-160
        int xB = SCREEN_W-PADDLE_W -
                 fix2int15(multfix15(filtBX+float2fix15(1.f),
                                     int2fix15(160)));               // 480-630
        int yA = gfix_to_pixel(filtAY);
        int yB = gfix_to_pixel(filtBY);

        paddleState.vXA = xA-lastxA;
        paddleState.vXB = xB-lastxB;
        paddleState.xA = xA;  paddleState.yA = yA;
        paddleState.xB = xB;  paddleState.yB = yB;

        lastxA = xA;  lastxB = xB;

        PT_YIELD_usec(1000);          // 1 kHz
    }
    PT_END(pt);
    // ---------- [END]  ----------
}
